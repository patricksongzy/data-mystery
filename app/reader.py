...
class Reader:
    ...
    def get_unknown_logs(self):
        logs = []
        
        for t, event in self.unknown_events.items():
            oneLog = 'at ' + t + ', ' + event.action + ' at ' + event.location
            logs.append(oneLog)
    ...
    def setup_people(self):
        ...
        for person in self.people.keys():
            self.people_logs[person] = self.logs_by_person(person)
        self.people_logs['Unknown'] = self.get_unknown_logs()
